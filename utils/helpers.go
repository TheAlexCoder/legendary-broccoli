...
// English words for recovery phrases (simplified version)
var recoveryWords = []string{
	...
	"crown", "cruel", "crush", "cry", "crystal", "cube", "culture", "cup",
	...
	"patch", "path", "patient", "patron", "pattern", "pause", "pave", "payment",
	...
	"replace", "report", "require", "rescue", "result", "return", "reveal", "review",
	"reward", "ride", "ridge", "rifle", "right", "rigid", "ring", "riot",
	...
	"wit", "wolf", "woman", "wonder", "wood", "wool", "word", "work",
...
func GenerateRecoveryPhrase() string {
	var words []string
	for i := 0; i < 6; i++ {
		n, err := rand.Int(rand.Reader, big.NewInt(int64(len(recoveryWords))))
		if err != nil {
			// Fallback to a simple random selection
			n = big.NewInt(int64(i % len(recoveryWords)))
		}
		words = append(words, recoveryWords[n.Int64()])
	}
	return strings.Join(words, " ")
}

func IsValidRecoveryPhrase(phrase string) bool {
	...
	for _, word := range recoveryWords {
		wordSet[word] = true
	}
